...
class Login extends Model
{
    ...
    {
        $sql     = "SELECT username,phone,id FROM   sc_user WHERE phone = :phone and userpassword = :userpassword";
        $db      = \Yii::$app->db;
        $userpassword = md5($post['userpassword'].'bhsj');
        $command = $db->createCommand($sql, [':phone'=>$post['phone'],':userpassword'=>$userpassword]);
