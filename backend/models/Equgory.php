...
class Equgory extends Model
{
	...
	public function catelist($get)
	{
        ...
        $sql     = "SELECT id,goryName,createtime FROM   ".$this->table." WHERE type=3 and ".$where;
        $db      = \Yii::$app->db;
        ...
        $sql     = "SELECT id,goryName,createtime FROM   ".$this->table." WHERE type=3 and ".$where." order by id desc limit ".$limit." , ".$pagesize;
        $command = $db->createCommand($sql);
	...
	{
        $sql     = "SELECT id,goryName,createtime FROM   ".$this->table." WHERE type=3";
        $db      = \Yii::$app->db;
        $command = $db->createCommand($sql);
	...
	{
        $sql     = "SELECT id,goryName,createtime FROM   ".$this->table." WHERE type=3 and id!=".$id;
        $db      = \Yii::$app->db;
        $command = $db->createCommand($sql);
        $data = $command->queryAll();
        return $data;
	...
	public function editdata($id){
        $sql     = "SELECT id,goryName,pid FROM   ".$this->table." WHERE type=3 and id= ".$id;
        $db      = \Yii::$app->db;
        $command = $db->createCommand($sql);
